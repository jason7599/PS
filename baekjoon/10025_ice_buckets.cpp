...
int n_buckets;
pii buckets[100'000]; // <pos, amount>
...
int main() {
    ...
    FOR(i, n_buckets) {
        input(buckets[i].se, buckets[i].fi);
    }

    sort(buckets, buckets + n_buckets);

    int ans = 0;
    for (int l = 0, r = 0, cur = 0; l < n_buckets; l++) {
        for (; r < n_buckets 
            && buckets[r].fi - buckets[l].fi <= range * 2; r++) {
            cur += buckets[r].se;
        }
        upmax(ans, cur);
        cur -= buckets[l].se;
