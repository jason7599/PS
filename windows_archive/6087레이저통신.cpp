...
int sizeX, sizeY, targetX, targetY;
int dist[100][100];
bool map[100][100];
// up, right, down, left
int dx[] = {-1, 0, 1, 0};
...
bool chck(int x, int y) {
    return !(x < 0 || x == sizeX || y < 0 || y == sizeY || map[x][y]);
}

// dijkstra
int minMirrors(int startX, int startY) {

    dist[startX][startY] = 0;
    priority_queue<ppp, vector<ppp>, greater<ppp>> pq;
    // {{mirrorCount, direction}, {x, y}};
