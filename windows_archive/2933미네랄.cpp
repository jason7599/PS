...
void spear_hit(int hit_x, int hit_y)
{
    ...
    for (int init_dir = 0; init_dir < 4; init_dir++)
    {
        ...
        if (!out_of_bounds(init_nx, init_ny) && minerals[init_nx][init_ny] && !visited[init_nx][init_ny])
        {
            ...
            int biggest_x_at_y[MAX_MAP_SIZE] = {0};
            int smallest_y = init_ny;
            int biggest_y = init_ny;
            ...
            while (queue_front < cluster.size())
            {
                ...
                if (cur_x > biggest_x_at_y[cur_y]) 
                {
                    biggest_x_at_y[cur_y] = cur_x;
                }
            ...
            {
                int fall_distance = get_fall_distance_from(biggest_x_at_y[y], y);
